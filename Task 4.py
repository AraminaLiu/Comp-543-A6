...
#use convolutional filter on inputX with number of filters = 8 and number of kernels = 10
tt_conv = tf.keras.layers.Conv1D(filters=8, kernel_size=10)(inputX) #batchsize*247*8
# the weight matrix that maps the inputs and hidden state to a set of values
W = tf.Variable(np.random.normal(0, 0.05, (hiddenUnits +hiddenUnits + 247, hiddenUnits)), dtype=tf.float32)
...
# unpack the convolutional filter results
sequenceOfLetters = tf.unstack(tt_conv, axis=2)
# now we implement the forward pass
currentState = initialState
num_timeTick = 0
past_states = {}
#use convolutional filter results as timeticks
for timeTick in sequenceOfLetters:
    past_states[num_timeTick] = currentState
    if num_timeTick < 10:
        inputPlusState = tf.concat([timeTick, currentState, initialState], 1)  #batchsize*(247+hiddenUnits + hiddenUnits)
        
    else:
        inputPlusState = tf.concat([timeTick, currentState, past_states[num_timeTick-10]], 1)
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    #
    # stack all of the text into a matrix of one-hot characters
        x = np.stack (testdata[i][1] for i in myInts.flat)
    #
    # and stack all of the labels into a vector of labels
