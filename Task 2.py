...
num_timeTick = 0
#create a dictionary to store past states
past_states = {}
for timeTick in sequenceOfLetters:
    past_states[num_timeTick] = currentState
    #if first ten timetick, use initialState instead of past ten timestick
    if num_timeTick < 10:
        inputPlusState = tf.concat([timeTick, currentState, initialState], 1)  #batchsize*(256+hiddenUnits + hiddenUnits)
    #concatenate timetick, currentstate and past ten timestick together    
    else:
